...
class PecaDAO(DAO):
    ...
    def update(self, peca: Peca):
        with self.conn:
            ...
            query = ("UPDATE pecas SET descricao = ?, imagem = ?, status_id = ?, custo_aquisicao = ?, "
                     "titulo = ?, preco = ? WHERE id = ?")
            data = (peca.descricao, peca.imagem, peca.status.id,
                    peca.custo_aquisicao, peca.titulo, peca.preco, peca.id)
